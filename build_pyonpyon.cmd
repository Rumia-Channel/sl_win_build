...
git checkout pyonpyon || exit /b 1

if exist "%BUILD_DIR%" rd /s /q "%BUILD_DIR%"
md "%BUILD_DIR%" || exit /b 1
cd /d "%BUILD_DIR%" || exit /b 1

cmake -G "NMake Makefiles" -DCMAKE_BUILD_TYPE=Release -DSL_VARIANT=pyonpyon .. || exit /b 1
nmake || exit /b 1

if not exist "%BUILD_DIR%\pyonpyon.exe" (
    echo [ERROR] pyonpyon.exe not found: "%BUILD_DIR%\pyonpyon.exe"
    exit /b 1
)

"%BUILD_DIR%\pyonpyon.exe"
